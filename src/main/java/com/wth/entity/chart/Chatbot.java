...
public class Chatbot {
 ...
 private ChartConfig chartConfig;
    public Chatbot(ChartConfig vconfig, String conversationId) {
        this.chartConfig = vconfig;
        this.conversationId = conversationId;
        this.parentId = UUID.randomUUID().toString();
        if (!chartConfig.getEmail().isEmpty() || (!chartConfig.getPassword().isEmpty()
                && !chartConfig.getSessionToken().isEmpty())) {
            System.out.println("refresh"+chartConfig);
            config.put("session_token",vconfig.getSessionToken());
            config.put("email",vconfig.getEmail());
            config.put("password",vconfig.getPassword());
            refreshSession();
        }
    }
